...
begin
 ...
 begin
	if (Reset = '1') then
		...
		-- TODO--
		-- Die Werte in SevenSegment werden mit der SevenSegment Clock geschrieben und bei schnellen Änderungen der Werte, lassen sich diese nicht lesen!
		-- Ändern Sie den Code so, dass die Wiederholrate der Siebensegmentanzeige gleich bleibt und sich nur die Werte langsamer  bei einer Frequenz von 3 Hz ändern!
		if EnSevenSegmentClock = '1' then
			if (SevenSegmentValue = x"FFFF") then -- Zur Darstellung "ASHA" bei dem eigentlichen Wert von "FFFF"
					 ...
					 SevenSegment(23 downto 16) <= SevenSegmentset(to_integer(unsigned(SevenSegmentValue(7 downto 4))));
					 SevenSegment(15 downto 8) <= SevenSegmentset(to_integer(unsigned(SevenSegmentValue(11 downto 8))));
					 SevenSegment(7 downto 0) <= SevenSegmentset(to_integer(unsigned(SevenSegmentValue(15 downto 12))));
					 SevenSegment(31 downto 24) <= SevenSegmentset(to_integer(unsigned(SevenSegmentValue(3 downto 0))));
			end if;
